...
if r2 > 0.7:
    ...
    # 需要做val的数据
    csv = open('liver_data/chip-albumin-vali.csv', encoding='utf-8')
    ...
    # 与训练模型数据流程类型 做标准化
    x_val_norm = pd.DataFrame(scaler.fit_transform(x_val), columns=columns)
    x_val_pred_norm = pd.DataFrame(scaler.fit_transform(x_val_pred), columns=columns)
    ...
    final.to_csv('liver_data_complement/chip-val-com.csv', index=False, encoding='utf_8_sig')
